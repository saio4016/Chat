...
/**
 * 翻訳サイト(Excite)のサービスを使って文字列を翻訳する 役割：チャットの入力を指定した言語に自動的に翻訳する
 *
 * @author 18024115
 */
public class WebTranslator {
    ...
    //=========================================================================
    /**
     * メインメソッド このクラス単体での動作確認用。完成版では使用しない
     *
     * @param args
     */
    ...
    //=========================================================================
    /**
     * 翻訳処理本体メソッド 文章(word)をbeforeからafterに翻訳する
     *
     * @param word 翻訳対象の文章
    ...
    public String translation(String word, int before, int after) {
        ...
        try {
            ...
            if (before == 2 || after == 2) {
                props.put("big5", "no");  // チャレンジ用(中国語の簡の選択)
            }
            if (before == 3 || after == 3) {
                props.put("big5", "yes"); // チャレンジ用(中国語の繁の選択)
            }
            // URLクラスにアクセス先情報を設定
